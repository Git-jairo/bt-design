...
> Historisch build-logboek voor het `hackathon26`-experiment. Bewaard bij het
> experiment zelf. Voor projectbrede richtlijnen, zie de root `CLAUDE.md`.

**Datum:** 3 juni 2026
**Project:** IVR Priority Queue Dashboard — Budget Thuis
**Publieke URL:** https://www.budgetthuis.design/experiments/hackathon26
(`/hackathon26` blijft werken via een permanente redirect)
...
Beller hoort welkomstboodschap + wachtmuziek (pause)
   ↓
Dashboard pollt /api/experiments/hackathon26/queue elke 10 seconden
   ↓
...
> Bron van de Twilio Functions: `scripts/experiments/hackathon26/`
> (`identify-and-queue.js`, `connected.js`).
...
### Pagina: `/experiments/hackathon26`
...
### API routes (experiment-scoped)

| Route | Methode | Functie |
|---|---|---|
| `/api/experiments/hackathon26/queue` | GET | Haalt pending tasks op uit TaskRouter |
| `/api/experiments/hackathon26/queue/[sid]` | DELETE | Cancelt een specifieke task |
| `/api/experiments/hackathon26/queue/[sid]/accept` | POST | Verbindt de agent door |
| `/api/experiments/hackathon26/queue/clear` | POST | Leegt de demo-wachtrij |
| `/api/experiments/hackathon26/simulate` | POST | Voegt gesimuleerde bellers toe |

### Polling strategie
- Elke 10 seconden haalt de pagina verse data op via `/api/experiments/hackathon26/queue`
...
5. **Twilio webhooks:** na het verplaatsen van de API-routes naar `/api/experiments/hackathon26/...` moeten de redirect-URL's in de Twilio Console worden bijgewerkt.
...
cd bt-design
npm run dev
# → http://localhost:3000/experiments/hackathon26
```
